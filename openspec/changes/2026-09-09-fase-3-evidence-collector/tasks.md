...
- [ ] Definir interfaces de entrada en `packages/evidence/src/evidence-collector.ts`:
...
- [ ] Métricas: mapear `MetricSample` a `InvestigationEvidenceRef` con `kind: 'metric'` y evaluar calidad mediante `evidence-quality`.
- [ ] Eventos: mapear `DeviceEvent` a `InvestigationEvidenceRef` con `kind: 'event'`.
- [ ] Topología: mapear `TopologyHop` a `InvestigationEvidenceRef` con `kind: 'topology'`.
- [ ] Historial confirmado: mapear `ConfirmedIncident` a `InvestigationEvidenceRef` con `kind: 'incident_history'`, delimitado con prefijo `[Contexto Histórico]`.
- [ ] Feedback previo: mapear adjudicaciones previas a `InvestigationEvidenceRef` con `kind: 'feedback'`.
- [ ] Ordenamiento determinista (fecha descendente, kind, id) y acotación a `MAX_INVESTIGATION_EVIDENCE_REFS` (64).

## 3.2.3 Exportación y Barrel

- [ ] Re-exportar `collectInvestigationEvidence`, tipos asociados y errores en `packages/evidence/src/index.ts`.

## 3.2.4 Pruebas unitarias TDD

- [ ] Crear `packages/evidence/tests/evidence-collector.test.ts` cubriendo:
...
- [ ] Ejecutar `pnpm turbo run build lint test` y confirmar 100% verde sin regresiones.
