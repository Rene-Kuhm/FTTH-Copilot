...
- [x] Verificar `git fetch origin` y `git status` limpios.
- [x] Registrar el SHA revisado en `docs/validation/fase-0-baseline.md`.

## 0.2 Inventariar superficies reutilizadas

- [x] Listar modelos Prisma (`Incident`, `DetectedAlert`,
...
- [x] Definir 25 IDs opacos + 3 timestamps en
...
- [x] Reemplazar 13 ocurrencias de `lint: "echo \"no lint configured\""`
      por `lint: "eslint ."`. Verificar 0 residuos.

## Gate 0 — Cierre

- [x] `pnpm --filter @ftth-copilot/shared test` verde.
- [x] `pnpm --filter @ftth-copilot/evidence test` verde.
- [x] `pnpm typecheck` (turbo) verde.
- [x] `pnpm lint` (turbo) verde.
- [x] `openspec/config.yaml` sin `echo "no lint configured"` literales.
- [x] Documentos de evidencia firmados en `docs/validation/`.
